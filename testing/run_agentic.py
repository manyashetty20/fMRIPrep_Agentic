#!/usr/bin/env python3
"""
testing/run_agentic.py
======================
Run the full agentic pipeline (all agents enabled).
"""
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Full agentic fMRIPrep evaluation suite.")
    p.add_argument(
        "--config",
        type=Path,
...
def main() -> int:
    ...
    for pid in participants:
        out_dir = cfg.output_dir_for("agentic_full", pid)
        out_dir.mkdir(parents=True, exist_ok=True)
        cmd = base_main_cmd(cfg, participant=pid, output_dir=out_dir)
        log = DEFAULT_RESULTS_ROOT / "logs" / "agentic_full" / cfg.dataset_id / f"{pid}.log"
        rc = run_command(cmd, dry_run=args.dry_run, log_file=None if args.dry_run else log)
        write_run_manifest(
            DEFAULT_RESULTS_ROOT / "manifests" / "agentic_full" / cfg.dataset_id / f"{pid}.json",
            {"suite": "agentic_full", "participant": pid, "output_dir": str(out_dir), "exit_code": rc},
        )
        codes.append(rc)
